...
SESP_ROOT = Path(__file__).resolve().parents[2] / "swarm_evolving_protocol"
...
class ShadowMissionResult:
    mission_id: str
    agent: str = "SHADOW"
    activation: str = "SHADOW_STANDALONE_V1"
    started_at: str = ""
    completed_at: str = ""
    stop_reason: str = ""
...
class ShadowAgent:
    ...
    def _fetch_adsb_bbox(self, bbox: dict[str, float], label: str) -> dict[str, Any]:
        ...
        try:
            with httpx.Client(timeout=12.0, headers={"User-Agent": "shadow-agent/standalone/1.0"}) as client:
    ...
    def _fetch_web_intel(self) -> list[dict[str, Any]]:
        ...
        for src in ordered:
            ...
            try:
                with httpx.Client(timeout=15.0, headers={"User-Agent": "shadow-agent/standalone/1.0"}, follow_redirects=True) as client:
